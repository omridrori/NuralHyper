...
def get_data():
    n_samples=int(np.ceil(np.abs(np.random.normal(5000,3000))))
    n_features=np.random.randint(0,150)
...
net = Net()
net.load_state_dict(torch.load(r'C:\Users\User\Desktop\מדמח\מודלימז\פרויקט חדש\rf7'))
...
def makeHyper():
  ...
  n_estimators=np.random.choice(np.array(list(range(0,100,1))))
  if n_estimators==0:
      n_estimators=1
...
for epoch in range(1,100000000):
    ...
    try:
        if(epoch%100==0):
            torch.save(net.state_dict(), r'C:\Users\User\Desktop\מדמח\מודלימז\פרויקט חדש\rf7')
        ...
        real_score= sklearn.model_selection.cross_val_score(
        model, X, y, cv=15).mean()
        real_score=torch.tensor(real_score)
